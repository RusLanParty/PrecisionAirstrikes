        ...
        void onTick(object sender, EventArgs e)
        {
            if (Main.bombsActive)
            {
                for (int i = 0; i < Main.bombs.Count; i++)
                {
                    ...
                    Main.bombs[i].Velocity = vel - offset2;
                    Main.bombs[i].MaxSpeed = 300f;
                    if (Main.bombs[i].HeightAboveGround <= 35f)
                       {
                        World.AddExplosion(Main.bombs[i].Position, ExplosionType.ExplosiveAmmo, 1.0f, 0.0f, Main.owner, true, false);
                    for (int j = 0; j <= 3; j++)
                        {
                            World.ShootBullet(Main.bombs[i].Position.Around(2f), Main.target, Main.owner, WeaponHash.CompactGrenadeLauncher, 100, -1);
                            Wait(5);
                        }
